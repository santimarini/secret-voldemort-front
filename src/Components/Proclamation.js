    ...
    const handleSubmit = async (e) => {
        try {   
            if (localStorage.getItem("email") === gameInfo.minister) {
                ...
                let response = await axios.put(`http://localhost:8000/cards/discard_min?card_id=${gameInfo.disc_card}&game_name=${gameInfo.game_name}`)
                setDisabled(true)
    ...
    useEffect(() => {
        async function getInfo() {
            try {
                ...
                let choosed_response = await axios.get(`http://localhost:8000/dirmin_elect?game_name=${gameInfo.game_name}`)
         ...
         <div class className="container">
             {localStorage.getItem("email") === gameInfo.minister &&
                 <div class className="notification">
                     ...
                     <select id="min_discarded" size=<var>gameInfo.cards.length</var>>
                         { gameInfo.cards.map((card) => <option value={card.id}> { card.loyalty } </option>) }
                     </select>
                     <button onClick={handleSubmit} disabled={disabled}> Discard </button>
                </div>
              }
             {minDiscarded && localStorage.getItem("email") === gameInfo.director && (
               <div>
                 <h3>You have to proclaim.</h3>
                 <p>Please choose a card to discard.</p>
                 <select id="dir_discarded" size=<var>gameInfo.cards.length</var>>
                     { gameInfo.cards.map((card) => <option value={card.id}> { card.loyalty } </option>) }
                 </select>
                 <button onClick={handleSubmit} disabled={disabled}> Discard </button>
               </div>
             ...
             {!minDiscarded && localStorage.getItem("email") === gameInfo.director && (
                <div>
                    The minister {gameInfo.minister} is proclaiming.
                 </div>
             )}
             {(localStorage.getItem("email") !== gameInfo.minister &&
